...
Saymore 继续首先是一个可靠的开源语音输入工具。MVP 只交付一个经过完整验证的默认本地 ASR 模型，以及用于证明 Provider 边界的有限云端路径，不建设模型市场、社区排名或在线模型目录。
...
- 在默认模型稳定后增加少量、由 Saymore 维护的兼容模型变体。
