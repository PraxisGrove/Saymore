...
Accessibility capability metadata. It does not read, print, or persist text from
the focused control.
...
After delivery, clearing or submitting the control is not a correction. Before
delivery the observer captures a bounded, transient reset value from the target.
It stops without reporting an edit when the control becomes empty, returns to
that value, exposes its placeholder through `AXPlaceholderValue`, or replaces an
unanchored whole-control value without enough continuity to attribute it to the
delivered text. Placeholder text must not replace history final text or
contribute automatic dictionary evidence.

The observer may report multiple stable corrections during one observation
window. History applies them in order so the latest valid correction is the
current final text. Resetting or submitting the control preserves the last valid
correction and ends observation.

On macOS, text exposed through `AXTextInputMarkedRange` or the Web accessibility
equivalent `AXTextInputMarkedTextMarkerRange` is an in-progress input method
composition, not a stable correction. The observer pauses its stability window
while that range exists and starts a fresh window after the input method commits
the final text.
...
The command talks to the already Accessibility-authorized Preview process over a
user-only Unix socket. The result contains only:
...
stability window, and confirm that one dictionary entry and one notification are
produced.
