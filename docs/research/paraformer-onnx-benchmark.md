...
It is a development benchmark, not the desktop application's production adapter.
...
The optional fifth argument limits the number of manifest rows for a smoke test.
Each run also writes a sibling `*.predictions.jsonl` file.
