...
use template_app::{
    ...
    StorageError, dictionary_comparison_key, dictionary_variant_key, normalize_language_tag,
};
...
pub(super) fn record(
    ...
    transaction
        .execute(
            "INSERT INTO term_observations(
             ...
             ON CONFLICT(dictation_id, language, canonical_key, variant_key) DO UPDATE SET
    ...
    transaction
        .execute(
            "INSERT INTO dictionary_candidates(
             ...
             ON CONFLICT(language, canonical_key, variant_key) DO UPDATE SET
    ...
    let id = dictionary::upsert_in_transaction(
        ...
        NewDictionaryEntry {
            ...
            variants: vec![normalized.recognized_as.clone()],
    ...
    transaction
        .execute(
            "DELETE FROM term_observations
             WHERE language = ?1 AND canonical_key = ?2 AND variant_key = ?3",
            params![
                normalized.language,
                normalized.canonical_key,
                normalized.variant_key
...
impl NormalizedObservation {
    fn new(observation: NewDictionaryObservation) -> Result<Self, StorageError> {
        ...
        let recognized_as = observation.correction.recognized_as.trim().to_owned();
        let canonical = observation.correction.canonical.trim().to_owned();
        if dictation_id.is_empty() || recognized_as.is_empty() || canonical.is_empty() {
        ...
        Ok(Self {
            ...
            variant_key: dictionary_variant_key(&recognized_as),
