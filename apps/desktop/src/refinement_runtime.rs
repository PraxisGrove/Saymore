...
impl RefinementRuntime {
    ...
    pub fn plan(&self) -> RefinementPlan {
        ...
        Self::plan_from_settings(&settings)
    }

    pub fn plan_from_settings(settings: &SaymoreSettings) -> RefinementPlan {
        let provider = settings.llm.chat_completions.clone();
    ...
    pub fn refine_final_transcript<F>(
        &self,
        ...
        F: FnOnce(),
    {
        ...
        let provider_output = matches!(processed.refinement, RefinementStatus::Completed)
...
impl RefinementPlan {
    pub fn mode(&self) -> RefinementMode {
        self.mode.clone()
    }
}
...
mod tests {
    ...
    fn disabled_plan_returns_the_transcript_without_a_provider_call() {
        ...
        let Ok(processed) = runtime.refine_final_transcript(
            ...
            FinalTextRequest::new("  原始文本。  ", RefinementMode::Disabled),
            disabled_plan(),
            || {},
        ) else {
        ...
        assert_eq!(None, processed.provider_output);
    }
    ...
    fn configured_plan_completes_through_the_provider() {
        ...
        assert_eq!(Some("原始文本。".to_owned()), processed.provider_output);
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let processed = runtime.refine_final_transcript(
            DictationSessionId::generate(),
            refinement_request(
    ...
    fn refinement_request(transcript: &str) -> FinalTextRequest {
        let mut request = FinalTextRequest::new(transcript, RefinementMode::Enabled);
        request.language = Some("zh-Hans".to_owned());
